...
class PlanWorkflowUseCase:
    ...
    async def plan(self, project_id: str, mode: str = "whole_graph", node_id: str | None = None) -> dict[str, Any]:
        try:
...
class RunWorkflowUseCase:
    ...
    async def run_selected(self, project_id: str, node_id: str):
        project = await self.load_project(project_id)
        return await self.execute(project=project, run_type="single_node", mode="selected", node_id=node_id)

    async def run_from_node(self, project_id: str, node_id: str):
        project = await self.load_project(project_id)
        return await self.execute(project=project, run_type="from_node", mode="from_node", node_id=node_id)

    async def run_all(self, project_id: str):
        project = await self.load_project(project_id)
        return await self.execute(project=project, run_type="whole_graph", mode="whole_graph")
    ...
    async def execute(self, project: Project, run_type: str, mode: str, node_id: str | None = None) -> dict[str, Any]:
        plan = build_workflow_plan(project=project, mode=mode, node_id=node_id)
        if plan["errors"]:
            raise ApplicationError(
                400,
                {"ok": False, "errors": plan["errors"], "warnings": plan["warnings"]},
            )
        ...
        if errors:
            raise ApplicationError(400, {"ok": False, "errors": errors, "warnings": warnings})
        if not node_ids:
            raise ApplicationError(
                400,
                {
